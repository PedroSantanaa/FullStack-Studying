...
function attSituation(situation: situation, planet: { situation: situation }) {
  planet.situation = situation;
}
function addSat(satName: string, planet: { sat: string[] }) {
  planet.sat.push(satName);
}
function removeSat(name: string, planet: { sat: string[] }) {
  planet.sat = planet.sat.filter((sat) => sat !== name);
}

function promptValidSituation() {
  let situationNow: situation;
...
function firstMenuOption() {
  ...
  const confirmation = confirm(
    `Confirma o registro do planeta ${planetName}?\n coordenadas: [${x},${y},${z},${w}]\nSituação: ${situation}`
  );
...
function thirdMenuOption() {
  ...
  if (planet) {
    const confirmation = confirm(
      `Confirma o satelite ${satName} no planeta ${planet.name} ?`
    );
...
function fourthMenuOption() {
  ...
  if (planet) {
    const confirmation = confirm(
      `Excluir o satelite ${satName} do planeta ${planet.name} ?`
    );
